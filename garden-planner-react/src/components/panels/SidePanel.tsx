...
const TABS=[
  {id:'beds' as Tab, icon:'🪴', lbl: 'Beds'}, 
  {id:'pots' as Tab, icon:'🏺', lbl: 'Pots'},
  {id:'plants' as Tab, icon:'🌱', lbl: 'Plants'},
  {id:'gardens' as Tab, icon:'📁', lbl: 'Gardens'},
  {id:'settings' as Tab, icon:'⚙️', lbl: 'Settings'}
];
  ...
  return(<div className={'side-panel'+(col?' is-collapsed':'')}>
    ...
    {!col&&<div className="side-panel__inner">
      <div className="side-panel__tabs">
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} className={'side-panel__tab'+(tab===t.id?' is-active':'')}>
            <span className="side-panel__tab-icon">{t.icon}</span>{t.lbl}</button>))}
      </div>
